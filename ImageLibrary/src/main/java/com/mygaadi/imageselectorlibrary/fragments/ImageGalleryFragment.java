 ...
 */
public class ImageGalleryFragment extends Fragment implements LoaderManager.LoaderCallbacks<List<BucketItemModal>>, OnItemClickListener {
    public static final String TAG = "ImageGalleryFragment";

    /**
     * List to contain the URI of the selected Images
     */
    private List<Uri> mSelectedImageList;
    ...
    public void onItemClicked(View v, int adaptePosition, int layoutPosition) {
        ...
        if (mImageSelectionMode == GalleryFolderWiseFragment.SINGLE_IMAGE) {
            ...
            if (data.isSelected()) {
                ...
                mSelectedImageList.add(data.getImagePath());
            } else {
                mSelectedImagesCount--;
                mSelectedImageList.remove(data.getImagePath());
            }
        }
